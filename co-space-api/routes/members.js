...
router.get("/members_get/:id/:firstname", async (req, res) => {
...
router.get("/members_get", async (req, res) => {
...
router.get("/members_get/:member_id/", async (req, res) => {
...
// --> No need to fix, as browser will always send a GET request. We need to us another client to test this out, such as Postman
router.delete("/members_delete/:member_id/", async (req, res) => {
  let member_id = req.params.member_id;
  const conn = await pool.getConnection();
  await conn.beginTransaction();
  try {
